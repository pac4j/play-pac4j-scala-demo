...
object Application extends ScalaController {
  ...
  def index = Action { request =>
    ...
    val urlSaml = getRedirectAction(request, newSession, "Saml2Client", "/?5").getLocation()
    val profile = getUserProfile(request)
    Ok(views.html.index(profile, urlFacebook, urlTwitter, urlForm, urlBA, urlCas, urlSaml)).withSession(newSession)
  }
